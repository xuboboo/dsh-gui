...
- 🛍️ **插件市场** — 设置 → 插件 → 市场：内置可选插件（跨会话记忆搜索、定时任务、持久 Bash 终端、时间上下文等）卡片式一键启停，模型侧能力即时热生效，改动持久化到本机 settings.yaml
- 🔁 **迭代系统** — 官方发版一键升级流水线（detect/upgrade + 6 补丁序列 + 验证门禁），详见 UPGRADE-GUIDE
...
### v1.2.0（2026-09-12，开发中）

- ⚡ **启动性能优化**：heal 自愈加装「安装指纹」短路——同版本启动跳过 500 包逐包字节验真（升级后首启自动恢复全量校验）；launcher 禁用 otel 遥测子树并启用 V8 编译缓存。实测热启动 12s（含全量 heal；稳态指纹短路后 ≤7s，升级后首启保持全量）
- 🛍️ **插件市场扩容至 6 款**：新增定时任务（schedule_create 等工具）、Cordis 运行时工具（高级：模型可自检运行时/动态挂载插件）、Exa 网页搜索（需自备 EXA_API_KEY）；市场页新增搜索框

### v1.1.0（2026-09-12）

- 🛍️ **新增「插件市场」** — 设置 → 插件 → 市场：内置可选插件卡片式一键启停（跨会话记忆搜索、提问工具、时间上下文、持久 Bash 终端等），分类筛选，改动自动持久化到 settings.yaml，模型侧能力即时热生效（宿主半侧动态挂载/卸载受管条目，绝不触碰其他来源的挂载）；带界面的插件刷新页面后显示。市场本体为 @dsh-gui/plugin-market 双半侧插件（宿主设置命名空间+浏览器市场页），并随包分发跨会话搜索等此前未上架的官方可选插件。
### v1.1.0（2026-09-12）
